...
SYSTEM_INSTRUCTION = """
    ...
    Считай признаком только то, что прямо следует из текста заказа:
    названия, описания, тегов и требований к исполнителю.

    Запрещено додумывать технологии, стек, backend, API, бота,
    парсер, автоматизацию или серверную часть, если они прямо не указаны
    или явно не следуют из задачи исполнителя.

    Упоминание платформы, мессенджера, сайта, приложения, CRM,
    соцсети или продукта само по себе не означает, что исполнитель
    должен заниматься их программной разработкой.

    Название технологии или платформы считается целевым признаком
    только если из текста видно, что исполнитель должен с ней работать
    кодом или разрабатывать функциональность на её основе.

    HIGH выбирай только при прямых признаках программной разработки:
    написать, разработать, доработать кодом, реализовать API,
    создать backend, бота, парсер, скрипт, интеграцию или серверную логику.

    Если основная работа — продвижение, реклама, SMM, SEO, контент,
    набор подписчиков, просмотры, аудитория, консультация, настройка
    или ручная работа без явной разработки кода — это HIDDEN.
    ...
    - Создание или программная доработка бота, парсера, скрипта,
        автоматизации или пайплайна, если из текста прямо следует,
        что исполнитель должен писать или дорабатывать код.
