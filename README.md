...
`Python` · `asyncio` · `PostgreSQL` · `SQLAlchemy` · `LLM API` · `Docker Compose`
...
На первом этапе система получает проекты из внешнего источника, отбрасывает уже известные, загружает страницы новых проектов, выполняет фильтрацию и AI-анализ, после чего сохраняет результат.
...
```mermaid
flowchart LR
    A[Источник проектов] --> B[Загрузка списка]
    B --> C[Проверка дублей]
    C --> D[Загрузка страниц]
    D --> E[Парсинг деталей]
    E --> F[Базовая фильтрация]
    F --> G[AI-анализ]
    G --> H[Приоритет и объяснение]
    H --> I[Сохранение в БД]
    I --> J[Чтение активных проектов]
    J --> K[Проверка актуальности]
    K --> L[Динамические данные]
    L --> M[Список для просмотра]
```
